...
import "gtk3" Graphics.UI.Gtk.Layout.Table
...
main = do
    ...
    set display [ entryEditable := False
                ...
                , entryText     := "0" ]
    table <- tableNew 7 5 False              -- (1)
    --tableSetHomogeneous table True  -- (2)
    let attach x y w h item = tableAttachDefaults table item x (x+w) y (y+h) -- (3)
        mkBtn = mkButton st display
    attach 0 0 5 1 display           -- (4)
    mkBtn "+"  (operator Addition)        >>= attach 3 6 1 1
    mkBtn "MC" id                         >>= attach 0 1 1 1   -- (5)
...
{-
    mkBtn2 "MC"  display >>= attach 0 1 1 1   -- (5)
    mkBtn2 "MR"  display >>= attach 1 1 1 1
    mkBtn2 "MS"  display >>= attach 2 1 1 1
    mkBtn2 "M+"  display >>= attach 3 1 1 1
    mkBtn2 "M–"  display >>= attach 4 1 1 1
    mkBtn2 "←"   display >>= attach 0 2 1 1
    mkBtn2 "CE"  display >>= attach 1 2 1 1
    mkBtn2 "C"   display >>= attach 2 2 1 1
    mkBtn2 "±"   display >>= attach 3 2 1 1
    mkBtn2 "√"   display >>= attach 4 2 1 1
    mkBtn2 "7"   display >>= attach 0 3 1 1
    mkBtn2 "8"   display >>= attach 1 3 1 1
    mkBtn2 "9"   display >>= attach 2 3 1 1
    mkBtn2 "÷"   display >>= attach 3 3 1 1
    mkBtn2 "%"   display >>= attach 4 3 1 1
    mkBtn2 "4"   display >>= attach 0 4 1 1
    mkBtn2 "5"   display >>= attach 1 4 1 1
    mkBtn2 "6"   display >>= attach 2 4 1 1
    mkBtn2 "*"   display >>= attach 3 4 1 1
    mkBtn2 "1/x" display >>= attach 4 4 1 1
    mkBtn2 "1"   display >>= attach 0 5 1 1
    mkBtn2 "2"   display >>= attach 1 5 1 1
    mkBtn2 "3"   display >>= attach 2 5 1 1
    mkBtn2 "–"   display >>= attach 3 5 1 1
    mkBtn2 "="   display >>= attach 4 5 1 2
    mkBtn2 "0"   display >>= attach 0 6 2 1
    mkBtn2 "."   display >>= attach 2 6 1 1
    mkBtn2 "+"   display >>= attach 3 6 1 1
-}
